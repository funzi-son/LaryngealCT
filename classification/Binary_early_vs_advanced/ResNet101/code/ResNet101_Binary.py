# =====================================================
# ResNet101 model for eaely vs advanced classification
# =====================================================
...
BATCH_SIZE   = 2
ACCUM_STEPS  = 4       # 4 x 2 = effective batch 8
MAX_EPOCHS   = 500
PATIENCE     = 20
LR           = 1e-4
N_FOLDS      = 5
...
def build_model(dropout_p=0.30):
    m = resnet101(spatial_dims=3, n_input_channels=1, num_classes=2, pretrained=False)
...
with open(Path(OUTPUT_ROOT) / "cv_summary_resnet101.json", "w") as f:
    json.dump(fold_summ, f, indent=2)
print("Saved CV summary:", Path(OUTPUT_ROOT) / "cv_summary_resnet101.json")
...
).to_csv(Path(OUTPUT_ROOT) / "test_predictions_resnet101.csv", index=False)

with open(Path(OUTPUT_ROOT) / "test_metrics_resnet101.json", "w") as f:
    json.dump(
