...
df = pd.read_csv(r"path\to\Ensemble\Binary\DenseNet121\Early_vs_Advanced_DenseNet121_ensemble_predictions.csv")  # y_true, prob_raw, prob_cal
y_true   = df.iloc[:, 4].values          
prob_raw = df.iloc[:, 5].values
prob_cal = df.iloc[:, 6].values         

# Load t_opt_ens from json
with open(r"path\to\Ensemble\Binary\DenseNet121\Early_vs_Advanced_DenseNet121_ensemble_test_metrics.json", "r") as f:
    metrics = json.load(f)
...
save_folder = r"path\to\Ensemble\Binary\DenseNet121"
