# ===================================================
# Custom 3D CNN for early vs advanced classification
# ===================================================
import os
import json
import numpy as np
...
IMAGE_DIR   = "/path/to/your/Cropped/Volumes/dir"
...
# =========================
# Data augmentations (define here so available globally)
# =========================
train_transforms = tio.Compose([
    tio.RandomFlip(axes=('LR',), p=0.5),
    tio.RandomAffine(scales=(0.95, 1.05), degrees=8, translation=2, p=0.3),
    tio.RandomGamma(log_gamma=(0.8, 1.2), p=0.2),
    tio.RandomNoise(mean=0, std=0.02, p=0.15),
])
...
def create_weighted_sampler(df, label_col):
    ...
    w = {k: 1.0 / max(1, v) for k, v in counts.items()}
    weights = df[label_col].map(w)
...
class OnTheFlyDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Reference the correct global transform variables
        transforms = train_transforms if self.augment else val_transforms
        subj = transforms(subj)
        return subj.img.data.float(), torch.tensor(y).long()

# ============================
# Custom 5-layer 3D CNN model
# ============================
...
def epoch_pass(model, loader, optimizer=None, loss_obj=None, scaler=None):
    ...
    if train:
        ...
        print(f"Train epoch duration: {duration:.1f}s | Peak GPU Mem: {max_mem:.2f} GB")
    else:
        with torch.no_grad():
            torch.cuda.reset_peak_memory_stats(device)
            start_time = time.time()
...
def temperature_scaling(logits_val, y_val, max_iter=100):
...
for fold, (idx_tr, idx_va) in enumerate(skf.split(trm, trm[LABEL_COL]), 1):
    ...
    if LOSS_TYPE == "wbce":
        ...
        loss_obj = WeightedBCE(pos_weight=pos_weight_fold)
    ...
    T = temperature_scaling(logits_val, y_val)
...
with open(Path(OUTPUT_ROOT) / "cv_summary_custom3dcnn.json", "w") as f:
    json.dump(fold_summ, f, indent=2)
print("Saved CV summary:", Path(OUTPUT_ROOT) / "cv_summary_custom3dcnn.json")
...
).to_csv(Path(OUTPUT_ROOT) / "test_predictions_custom3dcnn.csv", index=False)

with open(Path(OUTPUT_ROOT) / "test_metrics_custom3dcnn.json", "w") as f:
    json.dump(
