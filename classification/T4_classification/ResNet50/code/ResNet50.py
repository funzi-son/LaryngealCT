# ===================================================
# Resnet50 model for early vs advanced classification
# ===================================================
...
# =========================
# Helper functions (same as your ResNet18 code)
# =========================
...
def epoch_pass(model, loader, optimizer=None, loss_obj=None, scaler=None):
    ...
    if train:
        ...
        for i, (xb, yb) in enumerate(loader):
            ...
            losses.append(loss.detach().item() * ACCUM_STEPS)  # undo division for logging
...
with open(Path(OUTPUT_ROOT)/"cv_summary_resnet50.json", "w") as f:
    json.dump(fold_summ, f, indent=2)
print("Saved CV summary:", Path(OUTPUT_ROOT)/"cv_summary_resnet50.json")
...
te_loader = DataLoader(OnTheFlyDataset(tem, IMAGE_DIR, augment=False),
                       batch_size=BATCH_SIZE, shuffle=False,
                       num_workers=NUM_WORKERS, pin_memory=True)
...
pd.DataFrame({"TCIA_ID": tem["TCIA_ID"], "Filename": tem["Filename"], "Label": y_te,
              "Probability": p_te, "Calibrated_Probability": p_te_cal}).to_csv(
                  Path(OUTPUT_ROOT)/"test_predictions_resnet50.csv", index=False)

with open(Path(OUTPUT_ROOT)/"test_metrics_resnet50.json","w") as f:
