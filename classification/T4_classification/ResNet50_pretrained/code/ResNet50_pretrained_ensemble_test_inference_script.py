#==================================================
#ResNet50-pretrained ensemble test inference script
#==================================================
import os, json, time
...
OUTPUT_ROOT = r"path\to\Ensemble\T4_Classification\ResNet50_pretrained"

LABEL_COL    = "Label_T4"   # 1 = T4, 0 = NonT4
...
MODEL_NAME = "ResNet50_MedicalNet"

USE_MEDICALNET = True  # must match training
...
# ------------------
# Model: same as training (MedicalNetResNet50WithHead)
# ------------------
class MedicalNetResNet50WithHead(nn.Module):
    def __init__(self, dropout_p=0.3, num_classes=2, use_medicalnet=True):
        ...
        if use_medicalnet:
            ...
            self.backbone = torch.hub.load("Warvito/MedicalNet-models", "medicalnet_resnet50")
            in_features = 2048  # MedicalNet ResNet50 feature dim
        else:
            from monai.networks.nets import resnet50
            backbone = resnet50(spatial_dims=3, n_input_channels=1, num_classes=num_classes, pretrained=False)
            # drop final fc to get features
    ...
    def forward(self, x):
        feats = self.backbone(x)        # [B, C, D, H, W]
        pooled = self.avgpool(feats)    # [B, C, 1, 1, 1]
        out = self.head(pooled)         # [B, num_classes]
        return out

def build_model(dropout_p=0.3, num_classes=2, use_medicalnet=True):
...
# ------------------
# Load CV summary (you may need to rename your existing file)
# ------------------
cv_summary_path = Path(OUTPUT_ROOT) / "cv_summary_resnet50_pretrained.json"
...
for fold_entry in cv_summary:
    ...
    t_opt_fold = float(fold_entry["optimal_threshold"])
    T_fold     = float(fold_entry["temperature"])
    ...
    model = build_model(dropout_p=0.30, num_classes=2, use_medicalnet=USE_MEDICALNET).to(device)
...
t_opt_ens = float(np.mean([fe["optimal_threshold"] for fe in cv_summary]))
T_ens     = float(np.mean([fe["temperature"]        for fe in cv_summary]))
...
print("\n=== ENSEMBLE TEST RESULTS (ResNet50 MedicalNet pretrained T4) ===")
...
pred_path = out_dir / "T4_vs_Non_T4_ResNet50_pretrained_ensemble_predictions.csv"
...
metrics_path = out_dir / "T4_vs_Non_T4_ResNet50_pretrained_ensemble_test_metrics.json"
