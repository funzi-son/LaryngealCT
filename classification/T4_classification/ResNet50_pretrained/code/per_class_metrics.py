...
preds_file = Path(OUTPUT_ROOT) / "test_predictions_resnet50_TL.csv"  
...
fold_summary = json.load(open(Path(OUTPUT_ROOT) / "cv_summary_resnet50_TL.json"))
...
# ---- Save JSON ----
with open(Path(OUTPUT_ROOT) / "per_class_metrics_resnet50_TL.json", "w") as f:
    json.dump(metrics_perclass, f, indent=2)

# ---- Save CSV ----
csv_save_path = Path(OUTPUT_ROOT) / "per_class_metrics_resnet50_TL.csv"
...
print(f"✅ Saved per-class metrics:\n- {csv_save_path}\n- {OUTPUT_ROOT}/per_class_metrics_resnet50_TL.json")