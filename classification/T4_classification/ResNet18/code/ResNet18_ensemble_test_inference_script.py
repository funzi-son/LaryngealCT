#=========================================
#ResNet18 ensemble test inference script
#=========================================
import os, json
...
OUTPUT_ROOT = r"path\to\Ensemble\T4_Classification\ResNet18"
...
BATCH_SIZE   = 2
NUM_WORKERS  = 0
N_FOLDS      = 5      # how many folds you trained

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Device:", device)

# ------------------
# Helpers (copied from training code)
# ------------------
...
class OnTheFlyDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        subj = val_tf(subj)   # no augmentation at test time
...
# ------------------
# Load CV summary (t_opt and T per fold)
# ------------------
cv_summary_path = Path(OUTPUT_ROOT) / "cv_summary_resnet18.json"
...
# ------------------
# Run test inference for each fold
# ------------------
for fold_entry in cv_summary:
    ...
    if not y_list:  # first fold
        y_list.append(ys)
    else:
        # sanity check labels order is identical
...
y_te = y_list[0]
fold_raw_probs = np.stack(fold_raw_probs, axis=0)   # [n_folds, N]
fold_cal_probs = np.stack(fold_cal_probs, axis=0)   # [n_folds, N]
...
# Get global ensemble t_opt and T (mean across folds)
...
pred_path = out_dir / "T4_vs_Non_T4_ResNet18_ensemble_predictions.csv"
...
metrics_path = out_dir / "T4_vs_Non_T4_ResNet18_ensemble_test_metrics.json"
