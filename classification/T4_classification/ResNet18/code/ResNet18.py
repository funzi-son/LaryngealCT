...
def create_weighted_sampler(df, label_col):
    # inverse-frequency weights per class (handles imbalance)
...
# =============================================
# Dataset (on-the-fly loading to lower memory)
# =============================================
...
def build_model(dropout_p=0.30):
    m = resnet18(spatial_dims=3, n_input_channels=1, num_classes=2, pretrained=False)
...
# =============================================
# Train/eval utils (with gradient accumulation)
# =============================================
...
for fold, (idx_tr, idx_va) in enumerate(skf.split(trm, trm[LABEL_COL]), 1):
    print(f"\n===== Fold {fold}/{N_FOLDS} =====")
    fold_dir = Path(OUTPUT_ROOT)/f"fold_{fold}"; fold_dir.mkdir(parents=True, exist_ok=True)
    df_tr = trm.iloc[idx_tr].reset_index(drop=True)
    df_va = trm.iloc[idx_va].reset_index(drop=True)

    # Per-fold class stats + weights
    ...
    for epoch in range(1, MAX_EPOCHS+1):
        ...
        logs.append({"epoch":epoch, **{f"tr_{k}":v for k,v in trmtr.items() if k not in ["y","p"]},
                               **{f"va_{k}":v for k,v in vamtr.items() if k not in ["y","p"]}})

        print(f"Epoch {epoch:03d} | tr_loss={trmtr['loss']:.4f} "
    ...
    # Threshold (Youden J) + Temperature scaling
    p_val = np.load(fold_dir/"val_p.npy"); y_val = np.load(fold_dir/"val_y.npy")
    ...
    print(f"  Calibrated@topt | acc={m['acc']:.3f} | bal_acc={m['bal_acc']:.3f} "
...
print("Saved CV summary:", Path(OUTPUT_ROOT)/"cv_summary_resnet18.json")
...
te_loader = DataLoader(OnTheFlyDataset(tem, IMAGE_DIR, augment=False), batch_size=BATCH_SIZE,
...
print("\n=== TEST RESULTS ===")
res_default = summarize(y_te, p_te, 0.5);       pretty_print("Default@0.5     ", res_default)
res_topt    = summarize(y_te, p_te, t_opt);     pretty_print("Uncalib@t_opt   ", res_topt)
res_cal     = summarize(y_te, p_te_cal, t_opt); pretty_print("Calibrated@t_opt", res_cal)

pd.DataFrame({"TCIA_ID": tem["TCIA_ID"], "Filename": tem["Filename"],
              "Label": y_te, "Probability": p_te, "Calibrated_Probability": p_te_cal}).to_csv(
    Path(OUTPUT_ROOT)/"test_predictions_resnet18.csv", index=False)
